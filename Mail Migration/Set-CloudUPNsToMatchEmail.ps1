...
#Create Output File
$LogFileName = "Set-BroanUPNs-$(Get-date -f yyyy-MM-dd-ss).log"
New-Item -Path $LogFileName
Start-Transcript -Path $LogFileName

#Check for Microsoft Online PowerShell
$MicrosoftOnlinePowerShell = Get-MsolUser -MaxResults 1
if ($MicrosoftOnlinePowerShell -eq $null) {
    Write-Host "ERROR: This script must be run from Microsoft Online PowerShell." -Foreground Red
    Exit(1)
}
